...
type Props = {
  shareUrl: string;
  /** Short line shown in WhatsApp etc. */
  shareBody: string;
};

export function CampaignShareRow({ shareUrl, shareBody }: Props) {
  ...
  const wa = `https://wa.me/?text=${encodeURIComponent(`${shareBody} ${shareUrl}`)}`;
  ...
  return (
    <div className="flex flex-wrap items-center gap-3">
      ...
      <button
        ...
        style={{ borderColor: "var(--color-border)", color: "var(--color-text)", fontFamily: "var(--font-body), sans-serif" }}
      >
        {copied ? <Check className="h-4 w-4" style={{ color: "var(--color-success, #16A34A)" }} aria-hidden /> : <Copy className="h-4 w-4" aria-hidden />}
